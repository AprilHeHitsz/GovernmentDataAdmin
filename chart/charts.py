...
from event.models import Event, Street, Type, Property, Achieve, DisposeUnit, Community, EventSource, MainType, SubType
    ...
    def get_funnel_data(self):
        ...
        for street in self.street_list:
            ...
            for community in communities:
                length += len(Event.objects.filter(community=community, achieve=Achieve.objects.get(status="overtime")))
            if length == 0:
                continue
            the_funnel_data.append((street.name, length))

        return the_funnel_data

    def funnel_base(self) -> Funnel:
        data = self.get_funnel_data()
        c = (
            ...
            # .set_global_opts(title_opts=opts.TitleOpts(title="超时完成情况"))
            .dump_options_with_quotes()
        )
        return c
